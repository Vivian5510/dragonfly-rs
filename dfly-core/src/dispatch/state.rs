...
impl SlotStats {
    pub(super) fn increment(&mut self, slot: u16) {
        let index = usize::from(slot);
        self.key_counts[index] = self.key_counts[index].saturating_add(1);
    }

    pub(super) fn decrement(&mut self, slot: u16) -> usize {
        let index = usize::from(slot);
    ...
    #[must_use]
    pub(super) fn get(&self, slot: u16) -> usize {
        self.key_counts[usize::from(slot)]
    }

    pub(super) fn clear(&mut self, slot: u16) {
        self.key_counts[usize::from(slot)] = 0;
    }

    pub(super) fn set_count(&mut self, slot: u16, count: usize) {
        self.key_counts[usize::from(slot)] = count;
    }

    pub(super) fn set_memory_bytes(&mut self, slot: u16, bytes: usize) {
        self.memory_bytes[usize::from(slot)] = bytes;
    }

    pub(super) fn increment_write(&mut self, slot: u16) {
        let index = usize::from(slot);
        self.total_writes[index] = self.total_writes[index].saturating_add(1);
    }

    pub(super) fn increment_read(&mut self, slot: u16) {
        let index = usize::from(slot);
        self.total_reads[index] = self.total_reads[index].saturating_add(1);
    }

    pub(super) fn add_memory_bytes(&mut self, slot: u16, bytes: usize) {
        let index = usize::from(slot);
        self.memory_bytes[index] = self.memory_bytes[index].saturating_add(bytes);
    }

    pub(super) fn sub_memory_bytes(&mut self, slot: u16, bytes: usize) {
        let index = usize::from(slot);
...
impl StoredValue {
    #[must_use]
    pub(super) fn string_len(&self) -> usize {
        match self {
    ...
    #[must_use]
    pub(super) fn string_bytes(&self) -> &[u8] {
        match self {
    ...
    #[must_use]
    pub(super) fn clone_string_bytes(&self) -> Vec<u8> {
        self.string_bytes().to_vec()
    }

    #[must_use]
    pub(super) fn into_string_bytes(self) -> Vec<u8> {
        match self {
...
impl ValueEntry {
    #[must_use]
    pub(super) fn new_string(value: Vec<u8>, expire_at_unix_secs: Option<u64>) -> Self {
        Self {
...
impl DispatchState {
    ...
    pub(super) fn db_table_mut(&mut self, db: DbIndex) -> &mut DbTable {
        self.db_tables.entry(db).or_default()
    }

    /// Returns immutable table for one logical DB.
    pub(super) fn db_table(&self, db: DbIndex) -> Option<&DbTable> {
        self.db_tables.get(&db)
    }

    /// Returns immutable map for one logical DB.
    pub(super) fn db_map(&self, db: DbIndex) -> Option<&HashMap<Vec<u8>, ValueEntry>> {
        self.db_table(db).map(|table| &table.prime)
    }

    pub(super) fn remove_expire_deadline(table: &mut DbTable, key: &[u8], expire_at: u64) {
    ...
    pub(super) fn add_expire_deadline(table: &mut DbTable, key: &[u8], expire_at: u64) {
        let _ = table
            .expire_order
            .entry(expire_at)
            .or_default()
    ...
    pub(super) fn update_expire_index(table: &mut DbTable, key: &[u8], expire_at: Option<u64>) {
    ...
    pub(super) fn increment_slot_stat(table: &mut DbTable, key: &[u8]) {
    ...
    pub(super) fn increment_slot_write_stat(table: &mut DbTable, key: &[u8]) {
        table.slot_stats.increment_write(key_slot(key));
    }

    pub(super) fn increment_slot_read_stat(table: &mut DbTable, key: &[u8]) {
        table.slot_stats.increment_read(key_slot(key));
    }

    pub(super) fn slot_payload_bytes(key: &[u8], entry: &ValueEntry) -> usize {
        key.len().saturating_add(entry.string_len())
    }

    pub(super) fn account_upsert_slot_memory(
        table: &mut DbTable,
        key: &[u8],
    ...
    pub(super) fn account_remove_slot_memory(table: &mut DbTable, key: &[u8], removed: &ValueEntry) {
        table
            .slot_stats
            .sub_memory_bytes(key_slot(key), Self::slot_payload_bytes(key, removed));
    }

    pub(super) fn decrement_slot_stat(table: &mut DbTable, key: &[u8]) {
        let slot = key_slot(key);
    ...
    pub(super) fn upsert_key(&mut self, db: DbIndex, key: &[u8], value: ValueEntry) -> Option<ValueEntry> {
    ...
    pub(super) fn remove_key(&mut self, db: DbIndex, key: &[u8]) -> Option<ValueEntry> {
    ...
    pub(super) fn set_key_expire(&mut self, db: DbIndex, key: &[u8], expire_at: Option<u64>) -> bool {
    ...
    pub(super) fn slot_candidate_keys(table: &DbTable, slot: u16) -> Vec<Vec<u8>> {
    ...
    pub(super) fn reconcile_slot_membership(&mut self, db: DbIndex, slot: u16, live: &[Vec<u8>]) {
        let Some(table) = self.db_tables.get_mut(&db) else {
            return;
        };
        if live.is_empty() {
    ...
    pub(super) fn collect_live_slot_keys(&mut self, db: DbIndex, slot: u16) -> Vec<Vec<u8>> {
    ...
    pub(super) fn bump_key_version(&mut self, db: DbIndex, key: &[u8]) {
    ...
    pub(super) fn purge_expired_key(&mut self, db: DbIndex, key: &[u8]) {
    ...
    pub(super) fn pop_next_expired_key(table: &mut DbTable, now: u64) -> Option<Vec<u8>> {
